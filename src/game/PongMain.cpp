/**
 * @file PongMain.cpp
 * @brief Main entry point for 3D Pong game
 * @version 1.0.0
 */
...
using CreateEngineFunc = Engine* (*)(const EngineConfig&);
using DestroyEngineFunc = void (*)(Engine*);
...
int main(int argc, char* argv[]) {
...
#endif
        std::cerr << std::endl;
        std::cerr << "Make sure the engine library is built and available in the current directory." << std::endl;
        return 1;
    }
    ...
    config.enable_audio = false;  // Disable audio for simplicity
    config.enable_physics = false;  // We handle physics in game
    config.max_frames_per_second = 60;
    config.enable_validation_layers = false;
    config.enable_debug_renderer = false;
    ...
    // Create and initialize Pong game
    auto pong_game = std::make_unique<game::PongGame>(engine);

    if (!pong_game->initialize()) {
