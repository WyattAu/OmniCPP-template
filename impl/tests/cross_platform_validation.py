    ...
    def validate_build_artifacts(self, toolchain_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if build_dir.exists():
            ...
            for ext in ["exe", "dll", "lib", "so", "a"]:
                ...
                if files:
                    artifacts[ext] = [str(f.relative_to(self.workspace_dir)) for f in files]
            ...
            for file_path in files:
                ...
                if full_path.exists():
                    with open(full_path, "rb") as f:
                        artifact_hashes[file_path] = hashlib.md5(f.read()).hexdigest()
...
def main():
    ...
    validator = CrossPlatformValidator(workspace_dir)
