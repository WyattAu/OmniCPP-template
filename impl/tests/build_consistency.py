...
class BuildConsistencyChecker:
    ...
    def compare_build_outputs(self, results: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if comparison["time_comparison"].get("time_variance", float('inf')) < comparison["time_comparison"].get("acceptable_range", 0):
            passed_checks += 1
...
def main():
    ...
    checker = BuildConsistencyChecker(workspace_dir)
