...
class TestPlatformDetectionIntegration:
    ...
    def test_detect_platform_logs_detection(self, caplog) -> None:
    ...
    def test_detect_architecture_logs_detection(self, caplog) -> None:
    ...
    @patch('platform.machine', 'x86_64')
    def test_x86_64_architecture_detection(self) -> None:
        """Test x86_64 architecture detection."""
        platform_info = detect_platform()
        assert platform_info.architecture == "x86_64"
        assert platform_info.is_64bit is True

    @patch('platform.machine', 'arm64')
    def test_arm64_architecture_detection(self) -> None:
        """Test ARM64 architecture detection."""
        platform_info = detect_platform()
        assert platform_info.architecture == "ARM64"
        assert platform_info.is_64bit is True

    @patch('platform.machine', 'i386')
    def test_i386_architecture_detection(self) -> None:
...
class TestCompilerDetectionIntegration:
    ...
    def test_detect_compiler_logs_detection(self, caplog) -> None:
        ...
        with caplog.at_level(logging.INFO):
            ...
            if compiler_info:
                assert any(
                    "Available compilers:" in record.message
                    for record in caplog.records
                )
            else:
                assert any(
                    "No compiler found" in record.message
                    for record in caplog.records
                )
...
class TestTerminalSetupIntegration:
    ...
    def test_terminal_setup_on_windows(self, mock_detect_platform) -> None:
    ...
    @patch('omni_scripts.platform.detector.detect_platform')
    def test_terminal_setup_on_linux(self, mock_detect_platform) -> None:
        ...
        # Linux typically doesn't need special terminal setup
        # Verify platform detection works
        platform_info = detect_platform()
        assert platform_info.os == "Linux"

    @patch('omni_scripts.platform.detector.detect_platform')
    def test_terminal_setup_on_macos(self, mock_detect_platform) -> None:
        ...
        # macOS typically doesn't need special terminal setup
        # Verify platform detection works
        platform_info = detect_platform()
        assert platform_info.os == "macOS"
...
class TestCrossPlatformCompilationIntegration:
    ...
    @patch('omni_scripts.platform.detector.detect_platform')
    def test_cross_compile_to_linux(self, mock_detect_platform) -> None:
    ...
    @patch('omni_scripts.platform.detector.detect_platform')
    def test_cross_compile_to_wasm(self, mock_detect_platform) -> None:
...
class TestLoggingIntegration:
    ...
    def test_platform_detection_logging(self, caplog) -> None:
    ...
    def test_compiler_detection_logging(self, caplog) -> None:
        """Test compiler detection is logged."""
        with caplog.at_level(logging.INFO):
            compiler_info = detect_compiler()

            # Verify compiler detection was logged
            assert any(
                "Available compilers:" in record.message or
                "No compiler found" in record.message
                for record in caplog.records
            )

    def test_validation_logging(self, caplog) -> None:
