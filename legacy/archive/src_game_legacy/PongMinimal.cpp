...
// Minimal 3D Pong game to test basic functionality
// This version removes all complex features to isolate crash issues
...
public:
    ...
    int run() {
        ...
        while (!game_over) {
            ...
            auto elapsed = std::chrono::duration<float>(std::chrono::high_resolution_clock::now() - current_time).count();
            if (elapsed < frame_time) {
                std::this_thread::sleep_for(std::chrono::duration<float>(frame_time - elapsed));
            }
            
            // Run for a limited time for demo
            if (frame_count > 3600) { // ~60 seconds
...
int main() {
    ...
    std::cout << "Minimal version for testing" << std::endl;
    std::cout << std::endl;
